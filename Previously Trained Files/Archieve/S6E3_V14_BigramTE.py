"""
S6E3 V14 - Bi-gram / Tri-gram Categorical Target Encoding
================================================================================
Strategy: V12 baseline + composite categorical TE features (S6E2 1st place)

Key Idea:
  Concatenate pairs/triplets of TOP categoricals into composite strings,
  then target-encode them via inner K-Fold TE. This captures 2-way and 3-way
  categorical interactions that XGBoost can't easily learn from splits alone.

  E.g., Contract_InternetService = "Two year_Fiber optic" → TE mean

  Different from ORIG_proba (which maps single columns) and from EXP3
  cross-interactions (which used ORIG_proba on interactions → 0.96+ corr with raw).

Source: S6E2 (Heart Disease) 1st place winning solution.
...
# ═══════════════════════════════════════════════════════════════════════════════
# Top categoricals for bi-gram/tri-gram (by feature importance in V12)
# ═══════════════════════════════════════════════════════════════════════════════
...
if __name__ == "__main__":
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    # [2/4] Feature Engineering — V7 pipeline (same as V12)
    # ═══════════════════════════════════════════════════════════════════════════
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    # [3/4] NEW: Bi-gram / Tri-gram Composite Categoricals
    # ═══════════════════════════════════════════════════════════════════════════
    ...
    print(f"\n{'='*80}")
    print(f"V14 RESULTS — Bi-gram / Tri-gram Categorical TE")
    print(f"{'='*80}")
    ...
    if overall_auc > 0.91879 + 0.00010:
        print(f"\n🏆 V14 shows real improvement! Submitting with bi-gram/tri-gram TE.")
        # Save submission
        ...
        print(f"\nNo significant improvement. N-gram TE doesn't add value over V12.")
