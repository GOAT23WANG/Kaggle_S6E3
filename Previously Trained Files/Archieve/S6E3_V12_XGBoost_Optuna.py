"""
S6E3 V12 - XGBoost with Optuna-Optimized Hyperparameters
================================================================================
Strategy: V8 architecture + Optuna-tuned params from 93/100 trials (12h search)

Optuna Search Results (5-fold CV):
  V8 Baseline:  0.91844
  Optuna Best:  0.91879 (+0.00035) ← Trial 75
  
Key Parameter Changes vs V8:
  learning_rate:    0.05   → 0.0063  (8x lower, more trees)
  max_depth:        6      → 5       (shallower)
  colsample_bytree: 0.80   → 0.32   (60% less, heavy column dropout)
  gamma:            0.05   → 0.79   (more aggressive pruning)
  reg_alpha:        0.10   → 3.50   (35x more L1 regularization)
  reg_lambda:       1.00   → 1.29   (slightly more L2)
  min_child_weight: 5      → 6      (similar)
  subsample:        0.80   → 0.81   (similar)
"""
...
class CFG:
    VERSION_NAME = "v12"
    EXP_ID = "S6E3_V12_XGBoost_Optuna"
...
# ═══════════════════════════════════════════════════════════════════════════════
# Optuna-Optimized Parameters (Trial 75, Best of 93 trials, 5-fold CV = 0.91879)
# ═══════════════════════════════════════════════════════════════════════════════
XGB_PARAMS = {
    'n_estimators': 50000,
    'learning_rate': 0.0063,        # V8: 0.05   → 8x lower
    'max_depth': 5,                 # V8: 6      → shallower
    'subsample': 0.81,              # V8: 0.8    → similar
    'colsample_bytree': 0.32,       # V8: 0.8    → 60% less (key change!)
    'min_child_weight': 6,          # V8: 5      → similar
    'reg_alpha': 3.5017,            # V8: 0.1    → 35x more L1
    'reg_lambda': 1.2925,           # V8: 1.0    → slightly more L2
    'gamma': 0.790,                 # V8: 0.05   → 16x more pruning
...
if __name__ == "__main__":
    ...
    print("\nOptuna-Optimized Params vs V8:")
    v8 = {'learning_rate': 0.05, 'max_depth': 6, 'subsample': 0.8,
          'colsample_bytree': 0.8, 'min_child_weight': 5, 'reg_alpha': 0.1,
          'reg_lambda': 1.0, 'gamma': 0.05}
    for k in v8:
        v12_val = XGB_PARAMS[k]
        v8_val = v8[k]
        d = "↑" if v12_val > v8_val else "↓" if v12_val < v8_val else "="
        print(f"  {k:25s}: {v12_val:>10} {d} (V8: {v8_val})")
    ...
    print(f"\n[3/3] Training XGBoost ({CFG.N_FOLDS}-Fold CV, Optuna Params)...")
    ...
    print(f"V8 Baseline OOF: 0.91857")
    print(f"Improvement:     {overall_auc - 0.91857:+.5f}")
