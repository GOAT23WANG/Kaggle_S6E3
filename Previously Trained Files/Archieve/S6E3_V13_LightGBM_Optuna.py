"""
S6E3 V13 - LightGBM with Optuna-Optimized Hyperparameters
================================================================================
Strategy: V7 architecture + Optuna-tuned params from 50/100 trials (713 min search)

Optuna Search Results (5-fold CV):
  V7 Baseline:  0.91835
  Optuna Best:  0.91869 (+0.00034) ← Trial 44

Key Parameter Changes vs V7:
  learning_rate:     0.03   → 0.0122   (2.5x lower)
  max_depth:         6      → 11       (deeper trees)
  num_leaves:        31     → 30       (similar, but with deeper trees = sparser)
  colsample_bytree:  0.80   → 0.30     (63% less, heavy column dropout)
  min_child_samples: 20     → 19       (similar)
  reg_alpha:         0.10   → 7.16     (72x more L1!)
  reg_lambda:        1.00   → 5.44     (5.4x more L2)
  min_gain_to_split: 0.00   → 0.172    (NEW: split quality threshold)
...
class CFG:
    VERSION_NAME = "v13"
    EXP_ID = "S6E3_V13_LightGBM_Optuna"
...
# ═══════════════════════════════════════════════════════════════════════════════
# Optuna-Optimized Parameters (Trial 44, Best of 50 trials, 5-fold CV = 0.91869)
# ═══════════════════════════════════════════════════════════════════════════════
LGB_PARAMS = {
    'n_estimators': 10000,
    'learning_rate': 0.0122,        # V7: 0.03   → 2.5x lower
    'max_depth': 11,                # V7: 6      → deeper (but sparse via leaves)
    'num_leaves': 30,               # V7: 31     → similar (sparse for depth=11)
    'subsample': 0.66,              # V7: 0.8    → less
    'colsample_bytree': 0.30,       # V7: 0.8    → 63% less (key change!)
    'min_child_samples': 19,        # V7: 20     → similar
    'reg_alpha': 7.1583,            # V7: 0.1    → 72x more L1!
    'reg_lambda': 5.4400,           # V7: 1.0    → 5.4x more L2
    'min_gain_to_split': 0.172,     # V7: 0.0    → NEW: split quality gate
    'path_smooth': 8.89,            # V7: 0.0    → NEW: heavy path smoothing
...
if __name__ == "__main__":
    ...
    print("\nOptuna-Optimized Params vs V7:")
    v7 = {'learning_rate': 0.03, 'max_depth': 6, 'num_leaves': 31, 'subsample': 0.8,
          'colsample_bytree': 0.8, 'min_child_samples': 20, 'reg_alpha': 0.1,
          'reg_lambda': 1.0, 'min_gain_to_split': 0.0, 'path_smooth': 0.0}
    for k in v7:
        v13_val = LGB_PARAMS[k]
        v7_val = v7[k]
        d = "↑" if v13_val > v7_val else "↓" if v13_val < v7_val else "="
        print(f"  {k:25s}: {v13_val:>10} {d} (V7: {v7_val})")
    ...
    print(f"\n[3/3] Training LightGBM ({CFG.N_FOLDS}-Fold CV, Optuna Params)...")
    ...
    for i, (train_idx, val_idx) in enumerate(skf_outer.split(train, train[CFG.TARGET])):
        ...
        CAT_FEATURES_FINAL = [c for c in X_tr.columns if str(X_tr[c].dtype) == "category"]
        ...
        print(f"   Final Fold {i+1} AUC : {fold_auc:.5f} | {(time.time()-t0)/60:.1f} min")
    ...
    print(f"V7 Baseline OOF: 0.91851")
    print(f"V12 XGB OOF:     0.91892")
    print(f"Improvement:     {overall_auc - 0.91851:+.5f} vs V7")
