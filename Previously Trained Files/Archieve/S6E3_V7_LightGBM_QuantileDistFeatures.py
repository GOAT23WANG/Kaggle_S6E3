"""
S6E3 V7 - LightGBM with Inner KFold TE + Distribution + Quantile Distance Features
=====================================================================================
Strategy (V6 + 8 Validated Quantile Distance Features from EXP5):
...
7. **NEW: 8 Quantile Distance Features (EXP5)** — distance to Q25/Q50/Q75 of original churner/non-churner TotalCharges.
8. **LGBM GPU training** with dynamic Pseudo Labeling (only kept if PL AUC > Base AUC).
"""
...
class CFG:
    VERSION_NAME = "v7"
    EXP_ID = "S6E3_V7_LightGBM_QuantileDistFeatures"
...
if __name__ == "__main__":
    ...
    print("\n[Feature Engineering] Applying V6 + EXP5 Quantile Distance Features...")
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    print("  [V6] Adding 9 EXP3 Distribution Features...")
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    # 6. NEW: EXP5 Quantile Distance Features (8 validated, +0.00018 OOF over V6)
    # ═══════════════════════════════════════════════════════════════════════════
    print("  [NEW] Adding 8 EXP5 Quantile Distance Features...")
    
    # Quantile reference points from original churner/non-churner TotalCharges
    for q_label, q_val in [('q25', 0.25), ('q50', 0.50), ('q75', 0.75)]:
        ch_q = np.quantile(orig_churner_tc, q_val)
        nc_q = np.quantile(orig_nonchurner_tc, q_val)
        
        for df in [train, test]:
            # Distance to churner quantile
            df[f'dist_To_ch_{q_label}'] = np.abs(df['TotalCharges'] - ch_q).astype('float32')
            # Distance to non-churner quantile
            df[f'dist_To_nc_{q_label}'] = np.abs(df['TotalCharges'] - nc_q).astype('float32')
            # Gap: negative means closer to churners
            df[f'qdist_gap_To_{q_label}'] = (df[f'dist_To_nc_{q_label}'] - df[f'dist_To_ch_{q_label}']).astype('float32')
    
    # Only keep the 8 that survived greedy selection (not dist_To_ch_q25)
    QDIST_FEATURES = [
