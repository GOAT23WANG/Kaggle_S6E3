"""
S6E3 V6 - LightGBM with Inner KFold Target Encoding + EXP3 Distribution Features
==================================================================================
Strategy (V4 + 9 Validated Distribution Features from EXP3 v3):
...
6. **NEW: 9 Distribution Features** — percentile rank / z-score against original churner vs non-churner distributions.
7. **LGBM GPU training** with dynamic Pseudo Labeling (only kept if PL AUC > Base AUC).
"""
...
class CFG:
    VERSION_NAME = "v6"
    EXP_ID = "S6E3_V6_LightGBM_DistFeatures"
...
if __name__ == "__main__":
    ...
    print("\n[Feature Engineering] Applying V4 + EXP3 Distribution Features...")
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    # 5. NEW: EXP3 v3 Distribution Features (9 validated features, +0.00036 OOF)
    # ═══════════════════════════════════════════════════════════════════════════
    print("  [NEW] Adding 9 EXP3 Distribution Features...")
    ...
    # Original distribution references
    ...
    for df in [train, test]:
        ...
        # Percentile ranks against churner/non-churner/full distributions
        ...
        # Residual from InternetService group mean
        df['resid_IS_MC'] = (df['MonthlyCharges'] - df['InternetService'].map(orig_is_mc_mean).fillna(0)).astype('float32')
        
        # Conditional percentile rank: TotalCharges within InternetService group
        ...
        # Conditional percentile rank: TotalCharges within Contract group
        vals = np.zeros(len(df), dtype='float32')
